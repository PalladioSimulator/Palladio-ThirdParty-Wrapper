	...
	public String[] getEntries() {
		...
		if (source instanceof desmoj.core.advancedModellingFeatures.WaitQueue) {
			...
			desmoj.core.advancedModellingFeatures.WaitQueue wq = (desmoj.core.advancedModellingFeatures.WaitQueue) source;
		     ...
		     System.out.println(wq.getCooperationsCompleted());
			...
			// Title
			entries[12] = wq.getSlaveQueue().getName();
			// sQorder
			entries[13] = wq.getSQueueStrategy();
			// (Re)set
			entries[14] = wq.getSlaveQueue().resetAt().toString();
			// Completed
			entries[15] = Long.toString(wq.getCooperationsCompleted());
			// Qlimit
			entries[16] = Long.toString(wq.getSlaveQueue().getQueueLimit());
			if (wq.getSlaveQueue().getQueueLimit() == Integer.MAX_VALUE) {
				entries[16] = "unlimit.";
			}
			// Qmax
			entries[17] = Long.toString(wq.sMaxLength());
			// Qnow
			entries[18] = Long.toString(wq.sLength());
			// Qavg.
			entries[19] = Double.toString(wq.sAverageLength());
			// Zeros
			entries[20] = Long.toString(wq.sZeroWaits());
			// avg.Wait
			entries[21] = wq.sAverageWaitTime().toString();
            // avg.Coop
            entries[22] = wq.mAverageCoopTime().toString();
			// refused
			entries[23] = Long.toString(wq.getSRefused());
		} else {
