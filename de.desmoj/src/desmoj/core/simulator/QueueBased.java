...
public abstract class QueueBased extends desmoj.core.simulator.Reportable {
	...
	/**
	 * Defining a constant for the FIFO (First In First Out) service discipline 
	 * of the underlying queue: An Entity inserted into the queue is removed 
     * after all entities already enqueued with the same priority.
	 */
	public final static int FIFO = 0;

	/**
     * Defining a constant for the LIFO (Last In First Out) service discipline 
     * of the underlying queue: An Entity inserted into the queue is removed 
    ...
    /**
     * Defining a constant for the random service discipline 
     * of the underlying queue: An Entity inserted into the queue may be removed
     * before or after any other Entity with the same priority.
     */
	...
	public double averageLength() {
		...
		double avgLength = (_wSumLength + (_currentLength * TimeOperations.diff(
				now, _lastAcc).getTimeInEpsilon()))
	...
	protected void deleteItem(TimeInstant entryTime) {
		...
		_sumSquareWaitTime += waitTime.getTimeInEpsilon()
	...
	public double stdDevLength() {
		...
		if (TimeSpan.isEqual(deltaTime, TimeSpan.ZERO)) // 
			...
			return java.lang.Math.sqrt(java.lang.Math
					.abs((_wSumSquareLength + (len * len * spanSinceLastAcess
							.getTimeInEpsilon()))
	...
	public TimeSpan stdDevWaitTime() {
		...
		if (getObservations() > 0) {
			...
			return new TimeSpan(java.lang.Math.sqrt( // not nice but functual
					java.lang.Math
							.abs(((obs * _sumSquareWaitTime) - (mean * mean))
	...
	protected void updateStatistics() {
		...
		// length sum
		_wSumSquareLength += _currentLength * _currentLength
