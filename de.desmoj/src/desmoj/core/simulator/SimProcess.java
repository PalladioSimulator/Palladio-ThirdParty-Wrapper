...
import java.util.Vector; //TODO: oder dem Anwender �berlassen...?
 ...
 * the method <tt>lifeCycle()</tt>), they inherit from Entity and thus can also be used
...
public abstract class SimProcess extends Entity {
	...
	public SimProcess(Model owner, String name, boolean showInTrace) {
		...
		// set up simthread
		_myThread = new SimThread(getModel().getExperiment().getThreadGroup(),
				this);
     ...
     * <code>activate(new TimeSpan(0))</code>. The process will continue executing 
    ...
    public void activate() {
        ...
        // schedule this SimProcess
        getModel().getExperiment().getScheduler().schedule(this, null, new TimeSpan(0));
	...
	/**
	 * Schedules the sim-process to be activated at the given point in simulation
	 * time. This will allow a sim-process to continue executing its
	...
	public void cooperate() {
		...
		this._master = currentMaster; // set the master for this slave process
	...
	/**
	 * Passivates a sim-process for the given span of time. The simthread of this
	 * Sim-process is put into a lock and the scheduler, resp. the experiment's
	...
	public boolean isInterrupted() {
		return (_irqCode != null);
	}

	/**
	 * Returns the current running status of the sim-process. If a sim-process is
	 ...
	 * Sim-process' specific behaviour. This method starts after a sim-process has
	...
	public void obtainResources(Resource[] obtainedResources) {
		...
		for (int i = 0; i < obtainedResources.length; i++) {
			_usedResources.addElement(obtainedResources[i]);
		}
	...
	public void returnAllResources() {
		...
		while (!_usedResources.isEmpty()) {
			...
			Res crntResPool = ((Resource) _usedResources.firstElement())
					.getResPool();
			...
			for (int i = 1; i < _usedResources.size(); i++) {
				...
				if (((Resource) _usedResources.elementAt(i)).getResPool() == crntResPool) {
	...
	/**
	 * Sets the sim-process' slaveWaitQueue variable to the ProcessQueue in which
	...
	protected void setSupervisor(ComplexSimProcess complexProcess) {
		...
		this._supervisor = complexProcess;
	}
	 ...
	 * be called the first time a sim-process is supposed to start processing its
	 * <code>lifeCycle()</code> method.
	 */
    ...
    /**
     * As there is no generally appliciable means of cloning a SimProcess
     * (which would require cloning the execution state as well), this 
     * method returns a </code>CloneNotSupportedException</code>.
     * 
