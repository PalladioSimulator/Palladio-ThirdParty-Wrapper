...
public class Histogram extends desmoj.core.statistic.Tally {
	...
	public Histogram(Model ownerModel, String name, double lowerEnd,
		...
		this._lowerLimit = lowerEnd;
		this._upperLimit = upperEnd;
		this._cells = cells;

		checkParam(); // check the given parameters

		// set the rounded width of each cell
		this._width = round((_upperLimit - _lowerLimit)/ cells);

		// make a new table
		this._table = new long[cells + 2];
	...
	public void changeParameters(double low, double up, int cel) {
		...
		// set the rounded width of each cell
		this._width = round((_upperLimit - _lowerLimit) / _cells);

		if (_cells != oldCells) // number of cells has changed?
		{
			// make a new table
			this._table = new long[_cells + 2];
		}
	...
	protected void checkParam() {
		if (_cells <= 0) // the interval will not be devided into segments
		{
			sendWarning(
		...
		if (_lowerLimit > _upperLimit) // lowerLimit is greater than upperLimit
		{
			sendWarning(
					"Attempt to produce a Histogram about an interval, "
	...
	public double getLowerLimit(int cell) {
		...
		if (cell == 0) {
			...
			return StatisticObject.round(_lowerLimit + (cell - 1) * _width);
		}
	}
	...
	public int getMostFrequentedCell() {
		...
		for (int i = 0; i < _cells + 2; i++) // go through all cells
		{
			if (max < _table[i]) // new maximum frequency found?
			{
				max = _table[i]; // set the new highest frequency
	...
	public void update() {
		...
		double val = getLastValue() - _lowerLimit;

		int n = 0; // to which cell does the value belong to?

		if (_lowerLimit == _upperLimit) // no real limit given, all in one
		// segment
		{
		...
		} else // lowerLimit < upperLimit
		{
			if (val < 0.0) // underflow
			{
				...
				Double d = new Double(val / _width + 1);

				n = d.intValue(); // rounded down
			}
		}

		if (n > _cells + 1) // overflow?
		{
			_table[_cells + 1]++;
	...
	public void update(double val) {
		...
		// how much is the value above the lowerLimit?
		double diff = getLastValue() - _lowerLimit;
		...
		} else // lowerLimit < upperLimit
		{
			if (diff < 0.0) // underflow
			{
				...
				Double d = new Double(diff / _width + 1);
